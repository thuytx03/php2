...
@section('home-section')
    <div class="container">
        ...
        <form action="saveUpdateSalary" method="post" enctype="multipart/form-data">
            <div class="row">
            <input type="hidden" name="id" value="{{ $getOneSalary->id }}">
                ...
                <div class="form-group col-6 mt-3 ">
                    ...
                    <select name="staff_id" class="form-control" id="">
                        ...
                        @foreach($staffs as $value)
                            <option @if($value->id == $getOneSalary->staff_id) selected @endif value="{{$value->id}}">{{$value->full_name}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col-6 mt-3 ">
                    <label>Lương cơ bản:</label>
                    <input type="text" name="basic_salary" value="{{$getOneSalary->basic_salary}}" class="form-control" placeholder="Vui lòng nhập lương cơ bản">
                </div>
                <div class="form-group col-6 mt-3">
                    <label>Số ngày công:</label>
                    <input type="text" name="work_day" value="{{$getOneSalary->work_day}}" class="form-control" placeholder="Vui lòng nhập số ngày công">
                </div>
                <div class="form-group col-6 mt-3">
                    <label>Thưởng:</label>
                    <input type="text" name="bonus" value="{{$getOneSalary->bonus}}" class="form-control" placeholder="Vui lòng nhập thưởng">
                </div>
                <div class="form-group col-6 mt-3">
                    <label>Phụ cấp:</label>
                    <input type="text" name="allowance" value="{{$getOneSalary->allowance}}" class="form-control" placeholder="Vui lòng nhập phụ cấp">
